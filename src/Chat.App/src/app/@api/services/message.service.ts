...
export class MessageService implements IPagableService<Message> {
  ...
  getPage(options: { index: number; pageSize: number; }): Observable<EntityPage<Message>> {
    return this._client.get<EntityPage<Message>>(`${this._baseUrl}api/message/page/${options.pageSize}/${options.index}`)
  }
